...
##
# on my local machine
data_dir = '/data/data/open-bci/2023-09-28_demo'
scratch_directory = "/scratch"
print(f"CPU affinity: {os.sched_getaffinity(0)}")
##
ex_mat = scipy.io.loadmat(os.path.join(data_dir, '2023-09-28_demo_experiment1_recording3.mat'))
ex_mat

class OpenBCIDataset(torch.utils.data.Dataset):
    def __init__(self, directory):
        super().__init__()
        self.directory = directory
        self.files = glob.glob(os.path.join(directory, '*.mat'))
        # sort by trailing digits before extension, eg "recording1.mat" < "recording10.mat"
        # use regex
        key_fn = lambda x: int(re.search(r'\d+(?=\.mat)', x).group(0))
        self.files = sorted(self.files, key=key_fn)
        
        self.emg = []
        self.text = []
        for f in tqdm(self.files):
            mat = scipy.io.loadmat(f)
            self.emg.append(mat['raw_data'])
            self.text.append(mat['stimulus'][0])
            assert len(mat['stimulus']) == 1
     
dset = OpenBCIDataset(data_dir)
##
##
